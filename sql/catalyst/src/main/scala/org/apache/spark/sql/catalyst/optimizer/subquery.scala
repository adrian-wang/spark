    ...
    case Filter(condition, child) =>
      ...
      withSubquery.foldLeft(newFilter) {
        ...
        case (p, Not(InSubquery(values, ListQuery(sub, conditions, _, _)))) =>
          ...
          // (A.A1 = B.B1) AND (B.B2 = A.A2) AND B.B3 > 1
          // and filter table A with ISNOTNULL(A.A1) AND ISNOTNULL(A.A2)
          val nonNull = baseJoinConds.flatMap {
            case eq @ EqualTo(left, _) => Some(IsNotNull(left))
            case _ => None
          }.reduce(And)
          val finalJoinCond = (baseJoinConds ++ conditions).reduceLeft(And)
          Join(Filter(nonNull, outerPlan), newSub, LeftAnti, Option(finalJoinCond), JoinHint.NONE)
        case (p, predicate) =>
